...
namespace Dungeon
{
	...
	bool Texture::loadFromImage(const Image &image, GLint minMagFilter, GLint wrapMode)
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, getInteralFormat(image.getFormat(), true), (GLsizei)m_width, (GLsizei)m_height, 0, getInteralFormat(image.getFormat(), false), GL_UNSIGNED_BYTE, image.getPixelPtr());
